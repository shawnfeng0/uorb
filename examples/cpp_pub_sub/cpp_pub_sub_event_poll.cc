...
void thread_publisher_example_string() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        snprintf(reinterpret_cast<char *>(data.str), example_string_s::STRING_LENGTH, "%d: %s", i, "This is a string message (event poll). ");
...
void *thread_subscriber(uorb::EventLoop *loop) {
    loop->Listen<example_string_s>(uorb::msg::example_string, [](const example_string_s &msg) {
        LOGGER_INFO("[example_string] timestamp: %" PRIu64 ", msg: '%s'", msg.timestamp, msg.str);
    });
    loop->Listen<sensor_accel_s>(uorb::msg::sensor_accel, [](const sensor_accel_s &msg) {
        LOGGER_INFO("[sensor_accel] timestamp: %" PRIu64 ", accel: (%.2f, %.2f, %.2f), temp: %.2f", msg.timestamp, msg.x, msg.y, msg.z, msg.temperature);
    });
    loop->Listen<sensor_gyro_s>(uorb::msg::sensor_gyro, [](const sensor_gyro_s &msg) {
        LOGGER_INFO("[sensor_gyro] timestamp: %" PRIu64 ", gyro: (%.2f, %.2f, %.2f), temp: %.2f", msg.timestamp, msg.x, msg.y, msg.z, msg.temperature);
    });
    loop->Loop();
    return nullptr;
}
